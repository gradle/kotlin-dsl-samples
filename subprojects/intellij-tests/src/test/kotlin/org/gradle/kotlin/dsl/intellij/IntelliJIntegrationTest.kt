...
class IntelliJIntegrationTest : GradleImportingCodeInsightTestCase() {
    ...
    fun testBuildFileHighlighting() {
        ...
        runInEdtAndWait {
            openFileInEditor(buildFile)
        }

        runInEdtAndWait {
            updateScriptDependenciesSynchronously(buildFile, myProject)
        }
    ...
    fun kotlinScriptDefinitionFor(buildFile: VirtualFile): KotlinScriptDefinition? =
        ScriptDefinitionsManager.getInstance(myProject).findScriptDefinition(buildFile.name)
}
