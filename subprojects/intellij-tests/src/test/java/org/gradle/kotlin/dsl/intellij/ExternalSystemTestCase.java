...
public abstract class ExternalSystemTestCase extends UsefulTestCase {
    ...
    private void setUpInWriteAction() throws Exception {
        File baseDir = new File(myProject.getBaseDir().getPath());
        FileUtil.ensureExists(baseDir);
        myProjectRoot = LocalFileSystem.getInstance().refreshAndFindFileByIoFile(baseDir);
    }
